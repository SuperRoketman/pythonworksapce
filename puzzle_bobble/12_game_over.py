...
# 실패 : 바닥에 어떤 정해진 높이보다 버블이 내려오면 실패
...
class Pointer(pygame.sprite.Sprite):
    def __init__(self, image, position, angle):
        ...
        self.rect = image.get_rect(center=position)
        self.angle = angle
        self.original_image = image # 원본 이미지를 두지 않고 회전시키면 10도 회전 -> 10도 회전 (20도) 이게 10도 회전 -> 10도 회전(20도) => 30도회전 이렇게 되버릴 수 있음.
        self.position = position

    # sprite 에 있는 draw 함수를 쓰는 것처럼 보이기 위해...
...
def process_collision():
    ...
    if hit_bubble or curr_bubble.rect.top <= wall_height:
        row_idx, col_idx = get_map_index(*curr_bubble.rect.center) # 컬 버블 앞 *은 튜플을 언패키징 하는 것
